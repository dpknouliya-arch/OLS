...
$svgPath = "https://3d.jog-joinourgame.com/3djogdigital/admin/assets/svg/".$designId."/Calgary.svg";
...
foreach ($imageDecals as $imgDecal) {
    ...
    elseif (filter_var($imageUrl, FILTER_VALIDATE_URL)) {
        ...
        if ($imgData !== false) {
            ...
            $imageHref = "data:$mime;base64," . base64_encode($imgData);
        }
    }
    ...
    foreach ($displayMap[$display]['items'] as $targetId => $cfg) {
        ...
        $width  = !empty($cfg['width']) ? $cfg['width'] : 80;
        $height = !empty($cfg['height']) ? $cfg['height'] : 80;

        $img = $dom->createElementNS(
            "http://www.w3.org/2000/svg",
            "image"
        );
