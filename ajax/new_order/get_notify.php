...
$sql_select = "SELECT of_id,COUNT(chat_id) AS num_new FROM tbl_chat WHERE of_id IN (".$s_of_id.") AND is_read=0 AND chat_type='".$_POST["msg_type"]."' GROUP BY of_id; ";
...
while($row_num_new = $rs_num_new->fetch_assoc()){
	...
	$a_count_unread[($row_num_new["of_id"])] = $row_num_new["of_id"]."=".$row_num_new["num_new"];
}
