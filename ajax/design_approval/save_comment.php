...
if($conn->query($sql_insert)){
	...
	$sql_design_draft = "SELECT order_id FROM tbl_design_draft WHERE dd_id='".$dd_id."'; ";
	$rs_design_draft = $conn->query($sql_design_draft);
	$row_design_draft = $rs_design_draft->fetch_assoc();
	$order_id = $row_design_draft["order_id"];

	//---Connect to Lockerroom DB
	$conn3 = new mysqli($serverName,$userName,$userPassword,$dbName3);
	mysqli_set_charset($conn3, "utf8");

	$sql_order_head = "SELECT * FROM order_head WHERE order_id='".$order_id."'; ";
	$rs_order_head = $conn3->query($sql_order_head);
